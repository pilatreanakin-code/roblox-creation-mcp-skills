...
Fully automated pipeline that exports 3D meshes and textures from Blender to Roblox Studio. Blender MCP exports FBX + bakes textures → Open Cloud API uploads to Roblox → Roblox Studio MCP places everything with correct scale and position.

### [Blender Pro](./blender-pro)
Transforms the Blender MCP into a professional 3D artist. Deep user interviews, blueprint-based design, proper modeling techniques (modifiers, node materials, correct topology), visual self-critique, and multiple options mode.

## Compatibility

Built for **Claude Code**, but works with any AI coding tool that supports MCPs:
...
Clone the repo and copy the skill folders to your Claude Code skills directory:
