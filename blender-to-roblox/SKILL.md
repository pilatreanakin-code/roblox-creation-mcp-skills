...
Use `mcp__blender__execute_blender_code` to run Python (bpy) code that:

1. Identifies the target objects (selected objects, or all mesh objects if none selected)
2. For EACH object:
   a. Exports the mesh as `.fbx` to a temp directory
   b. Bakes/exports the diffuse texture as `.png` (if the object has materials with textures)
   c. Extracts the object's world transform (position, rotation, scale)
   d. **CRITICAL: Calculates the world-space bounding box size and center** using `obj.matrix_world @ Vector(corner)` for all 8 bounding box corners. This is the TRUE visible size of the object in the scene, accounting for parenting, modifiers, and non-applied scales. Without this, multi-part models (characters, vehicles, etc.) will have wrong proportions in Roblox.
   e. Extracts the material's base color (if no texture, just the color)
3. Returns a JSON manifest listing every exported file, its metadata, world_bbox_size, and world_bbox_center

Read `references/blender-export-code.md` for the exact bpy code templates to use.

**Critical Blender export settings for Roblox compatibility:**
- FBX export: `use_selection=True, mesh_smooth_type='FACE', add_leaf_bones=False, apply_scale_options='FBX_SCALE_ALL'`
- Texture bake: resolution 1024x1024 (default), format PNG
- Apply all modifiers before export
- Triangulate meshes before export (Roblox requires triangulated meshes)
