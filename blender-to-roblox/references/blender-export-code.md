...
This bakes the object's diffuse/base color to a texture image. Requires the object to have UV coordinates.
...
if obj is None:
    ...
    if not obj.data.uv_layers:
        print(f"WARNING: No UV map on '{obj_name}'. Generating automatic UVs.")
        bpy.ops.object.select_all(action='DESELECT')
        obj.select_set(True)
        bpy.context.view_layer.objects.active = obj
        bpy.ops.object.mode_set(mode='EDIT')
    ...
    # Create bake image
    img_name = f"{obj_name}_diffuse"
    if img_name in bpy.data.images:
        bpy.data.images.remove(bpy.data.images[img_name])
    img = bpy.data.images.new(img_name, resolution, resolution)

    # Assign image to all material nodes for baking
    ...
    # Save image
    filepath = os.path.join(export_dir, f"{obj_name}_diffuse.png")
    ...
    for mat_slot in obj.material_slots:
        ...
        if mat and mat.use_nodes:
            nodes = mat.node_tree.nodes
            bake_node = nodes.get("BakeTarget")
            if bake_node:
                nodes.remove(bake_node)

    print(f"OK: Baked texture to {filepath}")
```
